...
def review_hours(staff_id, password, student_id=None, request_index=None, action=None):
    ...
    if not student:
        ...
        return None
    pending = HoursCompleted.query.filter_by(student_id=student.id, status="pending").all()
    if request_index is None or request_index < 0 or request_index >= len(pending):
